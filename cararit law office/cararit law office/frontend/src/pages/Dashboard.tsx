...
import { ModeToggle } from "@/components/Modetoggle"; // 🌟 In-import ang Dark Mode Toggle
import {
  CalendarDays,
  Package,
  FileText,
...
const Dashboard = () => {
  ...
  // Fetch schedules AND tasks for the Notification Bar
  useEffect(() => {
        ...
        const pendingTasks = taskData
          .filter((t: any) => {
            ...
            const isAssignedToMe = 
              (assignedValue !== "" && assignedValue === myName) || 
  ...
  // SAFE CHECK: Prevent render if user data is still loading to avoid crashes
  if (!user) {
    return null;
  }
  ...
  const menuItems = [
    { 
      label: "Schedules", 
      description: "View or add upcoming events and appointments.", 
      icon: CalendarDays, 
      path: "/schedules", 
      iconColor: "text-blue-600 bg-blue-100 dark:bg-blue-500/20 dark:text-blue-400", 
      roles: ["admin", "scheduler"] 
    },
    { 
      label: "Inventory", 
      description: "Manage office equipment, supplies, and stocks.", 
      icon: Package, 
      path: "/inventory", 
      iconColor: "text-emerald-600 bg-emerald-100 dark:bg-emerald-500/20 dark:text-emerald-400", 
      roles: ["admin", "inventory_staff"] 
    },
    { 
      label: "Solicitation", 
      description: "Track incoming requests and solicitation letters.", 
      icon: FileText, 
      path: "/solicitation", 
      iconColor: "text-amber-600 bg-amber-100 dark:bg-amber-500/20 dark:text-amber-400", 
      roles: ["admin", "scheduler", "inventory_staff"] 
    },
    { 
      label: "My Tasks", 
      description: "Check and manage tasks assigned to you.", 
      icon: ListChecks, 
      path: "/tasks", 
      iconColor: "text-violet-600 bg-violet-100 dark:bg-violet-500/20 dark:text-violet-400", 
      roles: ["admin", "scheduler", "inventory_staff"] 
    },
    { 
      label: "Activity Logs", 
      description: "Monitor system history and user actions.", 
      icon: History, 
      path: "/logs", 
    ...
    { 
      label: "Manage Users", 
      description: "Add, edit, or remove system user accounts.", 
      icon: Users, 
      path: "/users", 
      iconColor: "text-rose-600 bg-rose-100 dark:bg-rose-500/20 dark:text-rose-400", 
      roles: ["admin"] 
    },
  ];
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 dark:bg-background relative font-sans">
      ...
      <header className="bg-primary px-4 md:px-6 py-4 flex flex-col md:flex-row items-center justify-between shadow-md relative z-50 gap-4 md:gap-0">
        <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start">
          ...
          <div className="text-center md:text-left">
            <h1 className="text-lg md:text-xl font-bold text-white tracking-tight">Carait Management System</h1>
        ...
        <div className="flex items-center justify-center gap-3 w-full md:w-auto">
          ...
          <div className="relative">
            <Button 
              variant="secondary" 
              size="icon" 
              className="h-10 w-10 md:h-11 md:w-11 rounded-xl relative hover:bg-white/90 transition-colors bg-white text-primary"
                  ...
                  <div className="max-h-[60vh] overflow-y-auto flex flex-col p-2 space-y-1 custom-scrollbar">
                    {notifs.length === 0 ? (
                      <div className="p-8 text-center flex flex-col items-center justify-center">
                        ...
                        <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">No upcoming events or pending tasks.</p>
                      </div>
                    ) : (
                      notifs.map((notif) => (
                        <div 
                          key={notif.id} 
                          className="flex items-start gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-slate-100 dark:hover:border-slate-800"
                          onClick={() => {
                            setShowNotifs(false);
                            navigate(notif.path);
                          }}
                          ...
                          <div className="flex flex-col text-left w-full">
                            ...
                            <span className="text-[11px] font-bold mt-1.5 inline-block w-fit px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800">
                              {!notif.hasDeadline ? (
                                ...
                                <span className="text-red-600 dark:text-red-400">⚠️ Overdue by {Math.abs(notif.diffDays)} days</span>
                              ) : notif.diffDays === 0 ? (
                                <span className="text-red-600 dark:text-red-400 animate-pulse">🚨 Due today!</span>
                              ) : (
          ...
          <Button
            variant="ghost"
            className="h-10 md:h-11 px-4 md:px-5 text-sm font-bold gap-2 rounded-xl text-white hover:bg-white/10 hover:text-white transition-colors"
            onClick={() => {
              logout();
              navigate("/");
            }}
      ...
      <main className="flex-1 p-4 md:p-10 flex flex-col max-w-6xl mx-auto w-full mt-4 md:mt-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {visibleItems.map((item) => (
            <button
              key={item.path}
              className="group flex flex-col items-start p-5 md:p-6 text-left bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl hover:shadow-md hover:border-primary/50 dark:hover:border-primary/50 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
            ...
            >
              <div className="flex items-center justify-between w-full mb-3 md:mb-4">
                ...
                <ChevronRight className="h-5 w-5 text-slate-300 dark:text-slate-600 group-hover:text-primary group-hover:translate-x-1 transition-all" />
              </div>
